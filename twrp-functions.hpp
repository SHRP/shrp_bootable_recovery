...
public:
	...
	static int Wait_For_Child(pid_t pid, int *status, string Child_Name, bool Show_Errors = true, bool onlyLOGInfo = false); // Waits for pid to exit and checks exit status, displays an error to the GUI if Show_Errors is true which is the default
